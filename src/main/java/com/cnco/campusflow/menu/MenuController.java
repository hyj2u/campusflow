...
public class MenuController {
    ...
    public ResponseEntity<?> addCartMenu(
            @Parameter(description = "메뉴 요청 정보") @RequestBody MenuRequestDto menuRequestDto, @AuthenticationPrincipal AppUserEntity appUser
    ) throws JsonProcessingException {
        Map<String, Object> data = new HashMap<>();
        data.put("menuId", menuService.addCartMenu(menuRequestDto, appUser).getMenuId());
        return ResponseEntity.status(HttpStatus.CREATED)
    ...
    @Operation(
            summary = "장바구니 메뉴 조회",
            description = """
                    사용자의 장바구니에 메뉴를 조회합니다.
                    
                    * JWT 인증이 필요합니다.
                    """
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "장바구니 메뉴 조회"),
            @ApiResponse(responseCode = "400", description = "잘못된 입력값"),
            @ApiResponse(responseCode = "401", description = "인증되지 않은 사용자")
    })
    @GetMapping("/cart")
    public ResponseEntity<?> addCartMenu(
        ...
        return ResponseEntity.status(HttpStatus.OK).body(CommonResponse.of(menuService.getCartList(appUser)));
    }
    ...
    @Operation(
            summary = "장바구니 전체 비우기",
            description = """
                    사용자의 장바구니 메뉴를 전체 삭제합니다.
                    
                    * Cart ID로 해당 장바구니를 삭제합니다.
                    * JWT 인증이 필요합니다.
                    """
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> deleteCart(
            @Parameter(description = "장바구니 번호", example = "1") @PathVariable Long cartId
    ) {
